...
export default function BottomNav({ currentUser }) {
  ...
  const tabs = [
    ...
    { path: '/map',       label: 'Bản đồ',  icon: 'ri-map-2-line' },
    ...
    <nav className="bg-white shadow-inner border-t flex justify-around py-2 fixed bottom-0 left-0 right-0 z-40">
      {tabs.map((t) => (
        <button
          ...
          className={`flex flex-col items-center px-1 transition-colors ${
            location.pathname === t.path ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'
          }`}
        >
