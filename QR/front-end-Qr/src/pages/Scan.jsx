...
export default function Scan() {
  ...
  const [roundStatus, setRoundStatus] = useState(null); // null | 'active' | 'none' | 'ended'
  const [roundName,   setRoundName]   = useState('');
  const [showAddBtn,  setShowAddBtn]  = useState(false);
  const [lastQr,      setLastQr]      = useState('');

  // ── Kiểm tra trạng thái đợt kiểm kê khi mount ───────────────
  useEffect(() => {
    const checkRound = async () => {
      try {
        // Lấy tất cả rounds rồi tự tìm active — backend không hỗ trợ filter ?status=
        ...
        if (!rounds.length) {
          ...
          setRoundStatus(
            latest.status === 'completed' || latest.status === 'closed'
              ? 'ended'
              : 'none'
          );
        }
  ...
  const onQrScanned = useCallback(
    async (qr) => {
      ...
      try {
        ...
        if (data?.round_no_round) {
          setRoundStatus('none');
          setScanResult('');
          showError(t("inv_round_status_none_title"));
          return;
        }

        // ── Đợt kiểm kê chưa bắt đầu ─────────────────────────
        if (data?.round_not_started) {
          setRoundStatus('none');
          setScanResult('');
          showError(t("inv_round_status_none_title"));
          return;
  ...
  useEffect(() => {
    // TK15BK / Zebra: ký tự đến nhanh (~5-10ms/key), timeout 400ms để chắc chắn
    ...
    const flushBuffer = () => {
      ...
      scanBuffer.current = '';
      if (!qr) return;
      // Loại bỏ ký tự điều khiển, CR/LF/Tab, ký tự non-printable
      qr = qr.replace(/[\r\n\t\x00-\x1F\x7F]/g, '').trim();
      // Zebra đôi khi prefix bằng dấu > hoặc suffix bằng dấu ;
    ...
    const handler = (e) => {
      // Enter hoặc Tab → flush ngay (cả Zebra lẫn generic wedge)
      ...
      // Bỏ qua phím điều hướng, modifier, F-keys
      if (e.key.length !== 1 || e.ctrlKey || e.altKey || e.metaKey) return;
      scanBuffer.current += e.key;
      lastInput.current = Date.now();
      clearTimeout(bufferTimer);
      // Auto-flush nếu không có thêm ký tự sau BUFFER_TIMEOUT ms
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col">
      ...
      <main className="flex-1 p-4 pb-20 text-center">
        ...
        {/* Banner tên đợt active */}
        {canScan && roundName && (
          <div className="mb-3 bg-green-50 border border-green-200 rounded-xl p-2 flex items-center justify-center gap-2">
            <span className="inline-block w-2 h-2 rounded-full bg-green-500 animate-pulse" />
            <span className="text-sm font-semibold text-green-700">📋 {roundName}</span>
          </div>
        )}

        {/* Loading */}
        {roundStatus === null && (
          <div className="text-center text-gray-400 py-10">{t("checking")}</div>
        )}

        {/* Banner khi không active */}
        {roundStatus !== null && !canScan && <RoundStatusBanner roundStatus={roundStatus} />}
            ...
            {showAddBtn && (
              <div className="mt-3 flex justify-center">
                <button
                  ...
                  className="px-5 py-2.5 bg-amber-500 text-white rounded-xl shadow hover:bg-amber-600 text-sm font-semibold"
                >
                  ➕ {t("add_new_device")}
                </button>
              </div>
        ...
        {/* Hidden input cho hardware scanner TC15BK */}
        <input
          ref={hiddenInputRef}
          tabIndex={0}
          style={{
